...
// ─── MEME ENGINE LOKAL — ffmpeg + ImageMagick fallback ──────────────────────
...
// Cek apakah ada font emoji di sistem
function findEmojiFont() {
  const candidates = [
    ...
    path.join(__dirname, "NotoColorEmoji.ttf"),
...
function runConvert(args) {
  return new Promise((resolve, reject) => {
    ...
    proc.on("close", (code) => {
      if (code === 0) resolve();
      else reject(new Error(`convert exit ${code}: ${stderr.slice(-500)}`));
    });
  });
}

// Buat meme pakai ImageMagick (support emoji via NotoColorEmoji)
// Ambil lebar gambar pakai ImageMagick "identify" supaya ukuran font
// bisa menyesuaikan (bukan fixed 48px yang bikin teks kecil di gambar besar).
function getImageWidth(inputPath) {
  return new Promise((resolve) => {
    const proc = spawn("identify", ["-format", "%w", inputPath]);
    let out = "";
    proc.stdout.on("data", (d) => (out += d.toString()));
    proc.on("error", () => resolve(null));
    proc.on("close", () => {
      const w = parseInt(out.trim());
      resolve(Number.isFinite(w) && w > 0 ? w : null);
    });
  });
}

async function createMemeImageMagick(imgBuffer, topText, bottomText) {
  const id = `${Date.now()}_${rand(1000, 9999)}`;
  ...
  const boldFont  = findMemeFont();

  // Skalakan pointsize berdasarkan lebar gambar (mirip rasio w/13 di ffmpeg),
  // dengan batas minimum/maksimum biar tetap wajar di gambar ekstrem kecil/besar.
  const imgWidth = await getImageWidth(inputPath);
  const pointsize = imgWidth ? Math.min(120, Math.max(28, Math.round(imgWidth / 11))) : 48;

  const args = [inputPath];

  if (boldFont) { args.push("-font", boldFont); }
  args.push("-pointsize", String(pointsize));

  const addText = (txt, gravity) => {
  ...
  if (topText)    addText(topText, "North");
  if (bottomText) addText(bottomText, "South");

  if ((hasEmoji(topText || "") || hasEmoji(bottomText || "")) && emojiFont) {
    args.splice(1, 0, "-font", emojiFont);
  }

  args.push("-quality", "92", outputPath);

  try {
    await runConvert(args);
    return fs.readFileSync(outputPath);
  } catch (e) {
    console.error("createMemeImageMagick error:", e.message);
    return null;
  } finally {
    try { fs.unlinkSync(inputPath); } catch {}
    try { fs.unlinkSync(outputPath); } catch {}
...
// Buat meme pakai ffmpeg (fallback, tidak support emoji)
async function createMemeFFmpeg(imgBuffer, topText, bottomText) {
  const id = `${Date.now()}_${rand(1000, 9999)}`;
  const inputPath  = path.join(MEME_TMP_DIR, `${id}.in.jpg`);
  const outputPath = path.join(MEME_TMP_DIR, `${id}.out.jpg`);
  fs.writeFileSync(inputPath, imgBuffer);
  ...
  const buildDraw = (txt, yExpr) => {
    const clean = txt.replace(/\p{Emoji}/gu, "").trim().toUpperCase();
    if (!clean) return null;
    let f = `drawtext=text='${escapeDrawtext(clean)}':fontcolor=white:fontsize=w/13:borderw=4:bordercolor=black:x=(w-text_w)/2:y=${yExpr}`;
    if (font) f += `:fontfile='${font.replace(/'/g, "\\'")}'`;
    return f;
  };

  const filters = [];
  if (topText)    { const d = buildDraw(topText, "20"); if (d) filters.push(d); }
  if (bottomText) { const d = buildDraw(bottomText, "h-text_h-20"); if (d) filters.push(d); }
  ...
  try {
    ...
    console.error("createMemeFFmpeg error:", e.message);
    return null;
  } finally {
    try { fs.unlinkSync(inputPath); } catch {}
    try { fs.unlinkSync(outputPath); } catch {}
...
// Fungsi utama — otomatis pilih engine terbaik
async function createMemeLocal(imgBuffer, topText, bottomText) {
  const needsEmoji = hasEmoji(topText || "") || hasEmoji(bottomText || "");
  const hasMagick  = await checkImageMagick();

  if (hasMagick) {
    const result = await createMemeImageMagick(imgBuffer, topText, bottomText);
    if (result) return result;
  }

  if (needsEmoji && !hasMagick) {
    console.warn("⚠️ Emoji di teks meme distrip karena ImageMagick tidak tersedia. Install: pkg install imagemagick");
  }

  return createMemeFFmpeg(imgBuffer, topText, bottomText);
}
...
async function handleSmeme(sock, msg, jid, text) {
  ...
  try {
    ...
    if (!memeBuffer) {
      await react("❌");
      return reply(sock, msg, "❌ Gagal membuat meme.\n\nPastikan sudah install:\n• *pkg install ffmpeg*\n• *pkg install imagemagick* (untuk emoji)");
    }

    const hasMagick = await checkImageMagick();
    const emojiNote = (hasEmoji(topText) || hasEmoji(bottomText)) && !hasMagick
      ? "\n\n⚠️ _Emoji distrip, install imagemagick untuk support emoji_"
      : "";

    await sock.sendMessage(jid, {
      image: memeBuffer,
      caption: `🎭 *Meme berhasil dibuat!*${emojiNote}`
    }, { quoted: msg });
    await react("✅");
  } catch (e) {

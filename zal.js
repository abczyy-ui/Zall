...
// ─── KONFIG GAME TEBAK/KUIS (lock antar-game & timeout jawaban) ───────────────
const GAME_TIMEOUT_MS = 120000; // 120 detik waktu menjawab
const GAME_KEYS = ["tebakBendera", "tebakKata", "kuis", "kuisMath", "kuisEnglish", "kuisJava"];
const GAME_LABELS = {
  tebakBendera: "🏳️ Tebak Bendera",
  tebakKata:    "🔤 Tebak Kata",
  kuis:         "📚 Kuis Umum",
  kuisMath:     "🔢 Kuis Math",
  kuisEnglish:  "🇬🇧 Kuis English",
  kuisJava:     "🏯 Kuis Jawa",
};
const GAME_SKIP_CMD = {
  tebakBendera: ".skip_bendera",
  tebakKata:    ".skip_kata",
  kuis:         ".skip_kuis",
  kuisMath:     ".skip_math",
  kuisEnglish:  ".skip_english",
  kuisJava:     ".skip_java",
};

// Cari apakah ada game/kuis lain yang masih aktif & belum dijawab di jid ini
function getActiveGame(state) {
...
function clearGameTimer(state, key) {
  if (state[key] && state[key].timer) {
    clearTimeout(state[key].timer);
  }
}

// Pesan saat waktu menjawab (120 detik) habis
function buildTimeoutMessage(key, data) {
  switch (key) {
    case "tebakBendera":
      return `⌛ *WAKTU HABIS!*\n\n🏳️ Jawaban: *${data.jawaban[0].toUpperCase()}* ${data.emoji}\n\nKetik *.tebakbendera* untuk main lagi!`;
    case "tebakKata":
      return `⌛ *WAKTU HABIS!*\n\n🔤 Jawaban: *${data.jawaban.toUpperCase()}*\n\nKetik *.tebakkata* untuk main lagi!`;
    case "kuis":
      return `⌛ *WAKTU HABIS!*\n\n✅ Jawaban: *${data.jawaban}*\n📖 ${data.explain}\n\nKetik *.kuis* untuk main lagi!`;
    case "kuisMath":
      return `⌛ *WAKTU HABIS!*\n\n🔢 Jawaban: *${data.jawaban}*\n\nKetik *.kuismath* untuk main lagi!`;
    case "kuisEnglish":
      return `⌛ *WAKTU HABIS!*\n\n🇬🇧 Jawaban: *${data.jawaban[0].toUpperCase()}*\n\nKetik *.kuisengglish* untuk main lagi!`;
    case "kuisJava":
      return `⌛ *WAKTU HABIS!*\n\n✅ Jawaban: *${data.jawaban}*\n📖 ${data.explain}\n\nKetik *.kuisjava* untuk main lagi!`;
    default:
      return "⌛ Waktu habis!";
  }
}

// Pasang timer 120 detik untuk sebuah game; otomatis dibatalkan/dihapus
// kalau dijawab benar atau di-skip sebelum waktunya habis.
function startGameTimeout(sock, jid, key) {
  const state = getState(jid);
  state[key].timer = setTimeout(() => {
    const s2 = getState(jid);
    if (s2[key]) {
      const data = s2[key];
      delete s2[key];
      sock.sendMessage(jid, { text: buildTimeoutMessage(key, data) }).catch(() => {});
    }
  }, GAME_TIMEOUT_MS);
}
...
async function handleTebakBendera(sock, msg, jid, sender) {
  ...
  if (active) return reply(sock, msg, `⚠️ Masih ada *${GAME_LABELS[active]}* yang belum dijawab!\n\nJawab dulu atau ketik *${GAME_SKIP_CMD[active]}* untuk skip.`);
  const soal = getRandomNoRepeat(BENDERA_DATA, jid, "bendera");
  state.tebakBendera = { ...soal, startedBy: sender };
  startGameTimeout(sock, jid, "tebakBendera");
  reply(sock, msg, `🏳️ *TEBAK BENDERA*\n\n${soal.emoji}\n\nNegara apa ini?\n⏱️ Waktu menjawab: *120 detik*\n💡 *.hint_bendera* | ⏭️ *.skip_bendera*`);
}
...
async function handleTebakKata(sock, msg, jid, sender) {
  ...
  startGameTimeout(sock, jid, "tebakKata");
...
async function handleKuis(sock, msg, jid, sender) {
  ...
  startGameTimeout(sock, jid, "kuis");
...
async function handleKuisMath(sock, msg, jid, sender) {
  ...
  startGameTimeout(sock, jid, "kuisMath");
...
async function handleKuisEnglish(sock, msg, jid, sender) {
  ...
  startGameTimeout(sock, jid, "kuisEnglish");
...
async function handleKuisJava(sock, msg, jid, sender) {
  ...
  startGameTimeout(sock, jid, "kuisJava");
